...
if (data.type === 'page_view') {
    ...
    if (url) {
        ...
        if (siteIdValue) {
            ...
            setCookie('rakuten_time_entered', makeString(Math.round(getTimestampMillis() / 1000)), options, false);
        }
        ...
        if (ranSiteIdValue) setCookie('rakuten_ran_site_id', makeString(ranSiteIdValue), options, false);
    }
    ...
    let requestUrl = 'https://'+enc(containerIdentifier)+'.'+enc(containerZone)+'.stape.io/stape-api/'+enc(containerApiKey)+'/v1/rakuten/auth-proxy';
    ...
    }, {method: 'POST', headers: {'Content-Type': 'application/json'}}, JSON.stringify(requestBody));
}
